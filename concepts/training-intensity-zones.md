...
description: The physiological backbone beneath every zone model — two thresholds dividing three intensity domains — and how the popular three-, five- and seven-zone systems, heart-rate percentages, RPE and the talk test map onto it.
...
- **Above the second threshold (hard / severe domain).** Lactate rises relentlessly and the effort is time-limited. This is the domain of [VO₂max](vo2max.md) intervals and racing short distances.
...
Within a model, the boundaries have to be pegged to something measurable. The common anchors are percentage of maximum heart rate, percentage of [heart-rate reserve](../techniques/heart-rate-and-effort-training.md) (Karvonen), percentage of lactate-threshold heart rate ([Friel](../sources/friel-lthr-zones.md)), percentage of VO₂max or maximal aerobic speed, rating of perceived exertion ([Chen et al. 2002](../sources/chen-2002-rpe-validity.md)), and the talk test, where the point at which full sentences become uncomfortable falls in the threshold region ([Quinn & Coons 2011](../sources/quinn-coons-2011-talk-test.md)). These do not line up exactly, because they are anchored differently: a percentage of maximum heart rate, a percentage of reserve and a percentage of threshold heart rate give three different target intensities. How to choose and apply them in practice, and why an estimated maximum heart rate is a weak anchor, is covered in [heart-rate and effort-based training](../techniques/heart-rate-and-effort-training.md).
...
The single most useful caveat is that the zone *boundaries* are operationalisations, not biological constants. The thresholds themselves are real and individual, but the percentages used to estimate them are population averages with wide scatter, and the fixed values (a 4 mmol/L lactate line, a "Zone 2 is 60 to 70% of max" rule) are conventions that misplace the boundary for many runners ([Faude et al. 2009](../sources/faude-2009-lactate-thresholds.md)). Anchoring zones to a [lactate or threshold field test](lactate-testing.md) places them far better than any age-based formula. And the popular idea that one narrow zone is uniquely magical, the [Zone 2](fat-oxidation-and-metabolic-flexibility.md) claim in particular, outruns the evidence ([Storoschuk et al. 2025](../sources/storoschuk-2025-zone2.md)): the zones are a navigation aid for distributing training, not a set of switches that each unlock a separate adaptation.

## Related

- [Lactate threshold](lactate-threshold.md)
- [Heart-rate and effort-based training](../techniques/heart-rate-and-effort-training.md)
- [Polarised training and 80/20](../techniques/polarised-training.md)
- [VO₂max](vo2max.md)
- [Critical speed and critical power](critical-speed-and-power.md)
- [Lactate testing](lactate-testing.md)
- [Fat oxidation and metabolic flexibility](fat-oxidation-and-metabolic-flexibility.md)
