...
The best-characterised risk factors come from a matched case-control study that found three independent associations: reduced ankle dorsiflexion, which was the strongest factor; obesity, with a body-mass index over 30 kg/m²; and spending the majority of the working day on the feet ([Riddle et al. 2003](../sources/riddle-2003-plantar-risk.md)). A tight calf and limited ankle dorsiflexion increase the tensile load the fascia has to absorb at each step, which fits the load-based model. Later work has found the dorsiflexion link less consistent than that early study suggested, so it is a plausible target rather than a proven cause ([Riddle et al. 2003](../sources/riddle-2003-plantar-risk.md)). For runners specifically, the usual trigger is a spike in training load, the same load-faster-than-tissue-tolerates story behind most [running injuries](../injury/running-injuries.md).
